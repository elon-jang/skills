# Product Image Extractor Skill v2.3

상품 페이지에서 **실제 상품 이미지만** 정확하게 추출하는 Claude Skill입니다.

## Description

Use this skill when the user asks to extract product images, get image URLs from a product page, or scrape product photos from e-commerce websites. This skill supports both curl-based (fast) and Playwright-based (bot-resistant) extraction methods to accurately filter and extract only actual product images, excluding backgrounds, banners, and logos. (user)
...
- **현재 버전**: v2.3
- **릴리스**: 2025-12-09
- **변경사항**: 메타 태그 폴백, 쿼리 파라미터 지원, HTML 엔티티 디코딩
